...
REPO_URL = 'https://github.com/Buffer0x7cd/Django-Test-driven-development.git'  
...
def _update_static_files(source_folder):
    run(
        'cd {0}'.format(source_folder) + ' && ../virtualenv/bin/python manage.py collectstatic --noinput'  
    )


def _update_database(source_folder):
    run(
        'cd {0} && ../virtualenv/bin/python manage.py migrate --noinput'.format(source_folder)
    )
